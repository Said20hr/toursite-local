        ...
        <div class="z-100 flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div class="flex items-center lg:order-2 z-100">
                @auth
                    <form method="POST" action="{{ route('filament.admin.auth.logout') }}">
                        @csrf
                        <button type="submit"  class="hidden xl:flex items-center gap-2
                        text-black bg-white hover:bg-gray-50 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 " >
                            <i class="fa-regular fa-circle-user"></i>
                            {{ __('Log out') }}
                        </button>
                    </form>                @endauth
                    @guest
                        <a href="{{ route('filament.admin.auth.login') }}" class="hidden xl:flex text-black bg-white hover:bg-gray-50 font-medium rounded-lg text-sm px-4 lg:px-4 py-2 lg:py-2.5 mr-2 ">{{__('Log in')}}</a>
                    @endguest
            ...
            <div class="z-100 xl:hidden bg-slate-900 mt-1 px-4 justify-between items-center w-full lg:flex lg:w-auto lg:order-1" style="display: none" x-show="open">
                <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-16 lg:mt-0">
                    ...
                    <li class="mb-6">
                        <a href="{{ route('filament.admin.auth.login') }}" class="bg-white text-secondary font-medium rounded-lg text-sm px-4 py-2">{{__('Log in')}}</a>
                    </li>
