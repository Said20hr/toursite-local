<x-guest-layout>
    <secion class="bg-white">
        <img alt="img" src="{{asset('storage/'.$service->image)}}" class="w-full object-cover object-center relative h-full xl:h-screen w-full">
        <div class="bg-background-dark bg-opacity-40 z-50 w-full flex flex-col justify-center text-center absolute inset-0 blur-sm h-60 xl:h-screen"></div>
    ...
    <section class="py-20 bg-gray-200 w-full">
        <div class="grid xl:grid-cols-1 gap-12 container">
             <div>
        ...
        <div class="container py-16 relative" >
            ...
            <div class="my-24 text-center">
                <p class="text-2xl mb-5">Custom your {{$service->title}} tour with us inquire now</p>
                <a href="{{route('booking.service', ['slug' => $service->slug, 'locale' => app()->getLocale()] )}}" class="text-white bg-elementary rounded-2xl px-12 py-2.5 text-xl mt-4">INQUIRY</a>
            </div>
        </div>
    </section>
